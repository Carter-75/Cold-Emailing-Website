...
@Component({
  ...
  template: `
    <div *ngIf="auth.showAuthModal()" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
      <!-- Backdrop -->
      <div (click)="close()" class="absolute inset-0 bg-black/60 backdrop-blur-md"></div>
      
      <!-- Modal Content -->
      <div class="auth-modal relative w-full max-w-md overflow-hidden rounded-[2.5rem] border border-white/10 bg-charcoal/80 p-10 shadow-2xl backdrop-blur-2xl">
