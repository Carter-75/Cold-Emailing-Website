...
router.get('/unsubscribes', verifyToken, async (req, res) => {
  try {
...
router.get('/unsubscribes/status', verifyToken, async (req, res) => {
  try {
    const user = await User.findById(req.user._id);
    const recipient = user.config?.testRecipientEmail || user.config?.senderEmail || '';
...
router.delete('/unsubscribes', verifyToken, async (req, res) => {
  try {
    const user = await User.findById(req.user._id);
    const recipient = user.config?.testRecipientEmail || user.config?.senderEmail;
...
router.post('/sync-inbox', verifyToken, async (req, res) => {
  try {
    const user = await User.findById(req.user._id);
    if (!user) return res.status(404).json({ message: 'User not found' });
