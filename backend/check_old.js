...
async function run() {
  try {
    ...
    const leads = await Lead.find({ userId: user._id, status: 'finished' }).sort({ updatedAt: 1 }).limit(5);
    console.log('--- OLDEST FINISHED LEADS ---');
    leads.forEach(l => console.log(`- ${l.businessName} (${l.status}) - ${l.createdAt}`));
